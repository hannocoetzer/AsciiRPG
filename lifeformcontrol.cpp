...
bool lifeformcontrol::start_gamecontrol(int temp_creaturecount)
{
	...
	}while(creatures[0]->getalive() == true & creatures[0]->getadvance() == false);
			...
			if(creatures[x]->getalive() == true)
			{
				...
				if(creatures[0]->getpos_x() == creatures[x]->getpos_x())
				{
					if(creatures[0]->getpos_y() == creatures[x]->getpos_y())
					{
						...
						int num = rand()%creatures[x]->getagility()+1;    // Get random between 1 and MAX_RANGE
						if(num <= 10)
						{
...
void lifeformcontrol::attack()
{
	...
	if(checkhealth())
	{
		creatures[0]->sethitpoints(creatures[0]->gethitpoints() - creatures[creature_collision_no]->getweapondamage());
		creatures[creature_collision_no]->sethitpoints(creatures[creature_collision_no]->
		gethitpoints()-creatures[0]->getweapondamage());
  	}
...
bool lifeformcontrol::checkhealth()
{
	...
	if(creatures[creature_collision_no]->gethitpoints() < 1 )
	{
		...
		creatures[0]->addweapon(creatures[creature_collision_no]->
		getweaponname(),creatures[creature_collision_no]->getweapondamage());
		creatures[creature_collision_no]->kill();

		flag = false;
		hero_detail();		//this is to show that the hero got the killed creatures weapon
	...
	if(creatures[0]->gethitpoints() < 1 )
	{
			...
			strcpy(str,"\nThe <");
			strcat(str,creatures[creature_collision_no]->getname());
			strcat(str,"> have slaugtered you.RIP RIP RIP!!! .Press [e] to exit...\n\n");
			waddstr(status_win,str);
			wrefresh(status_win);
